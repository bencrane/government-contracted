...
export async function deliverSubmission(payload: SubmissionPayload) {
  ...
  if (!webhookUrl) {
    console.log(
      `[govcon/submission] webhook not configured — logging only`,
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `[govcon/submission] webhook returned ${response.status}`,
        await response.text(),
      );
      return {
        delivered: false,
    ...
    console.error(`[govcon/submission] webhook fetch failed`, err);
