...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    title: `${c.title} — Government Contracted`,
    description: c.blurb,
  };
}
            ...
            <div className="md:col-span-5">
              <div className="border border-line-strong bg-white p-7 shadow-[0_24px_60px_-30px_rgba(15,26,46,0.18)]">
                ...
                <Link
                  ...
                  className="mt-6 inline-flex w-full items-center justify-center gap-2 bg-navy-700 px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-navy-800"
                >
                  Claim Your Entity
        ...
        <div className="mx-auto max-w-7xl px-6 py-16 md:py-20">
          <div className="grid gap-12 md:grid-cols-12 md:gap-16">
            <div className="md:col-span-4">
              <h2 className="font-display text-3xl leading-tight text-navy-900 md:text-4xl text-balance">
                What you get.
              </h2>
            </div>
      ...
      <section className="border-b border-line bg-slate-100">
        <div className="mx-auto max-w-7xl px-6 py-16 md:py-20">
          <div className="grid gap-12 md:grid-cols-12 md:gap-16">
            ...
            <ul className="md:col-span-8 space-y-4 text-[17px] leading-relaxed text-slate-800">
              {c.goodIf.map((item) => (
                <li key={item} className="flex gap-3">
                  <span className="mt-2 inline-flex h-1.5 w-1.5 flex-none rounded-full bg-copper-500" />
                  <span>{item}</span>
                </li>
              ))}
            </ul>
