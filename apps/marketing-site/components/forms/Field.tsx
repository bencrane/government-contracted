...
export function Field({ name, label, hint, error, required, children }: FieldProps) {
  return (
    <div>
      ...
      {error && (
        <p className="mt-1.5 text-xs font-medium text-copper-700">{error}</p>
      )}
    </div>
  );
}
...
export function TextInput({
  ...
  return (
    <input
      ...
      className={`${baseInput} ${invalid ? "border-copper-500" : ""}`}
    />
  );
}
...
export function TextArea({
  ...
  return (
    <textarea
      ...
      defaultValue={defaultValue}
      aria-invalid={invalid}
      className={`${baseInput} resize-none ${invalid ? "border-copper-500" : ""}`}
    />
  );
}
